...
| `monthly.py` | Monthly trend piece (PRD 12): reads `history.json` as a step function (a change is dated on the week file that reported it, else a calibration break, else the first run after the previous snapshot's last confirmation), computes bloc series / dimension movement / movers, one structured-output request over the month's digest items (sections citing any other URL are dropped), writes `YYYY-MM.json` |
