...
- The plans were written for the project's architecture at the time: vanilla JS + D3, centralized state store, Vite build. The frontend is now vanilla TypeScript (strict, no framework), with state writes going through the intents in `src/state/interactions.ts`
